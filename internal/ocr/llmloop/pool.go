...
func (p *CommentWorkerPool) Submit(ctx context.Context, f func() ([]model.LlmComment, error)) {
	p.submit(ctx, f, nil)
}
...
func (p *CommentWorkerPool) SubmitFor(ctx context.Context, key string, f func() ([]model.LlmComment, error)) {
	p.keysMu.Lock()
	if p.keys == nil {
...
func (p *CommentWorkerPool) submit(ctx context.Context, f func() ([]model.LlmComment, error), kwg *sync.WaitGroup) {
	p.wg.Go(func() {
		if kwg != nil {
			defer kwg.Done()
		}
		...
		defer func() {
			if r := recover(); r != nil {
				fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] CommentWorkerPool panic: %v\n%s\n", r, debug.Stack())
			}
		}()

		comments, err := f()
		if err != nil {
			fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] CommentWorkerPool error: %v\n", err)
		}
		p.resultsMu.Lock()
