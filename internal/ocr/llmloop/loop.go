...
import (
	...
	"github.com/Audi-dask/Overseer/internal/ocr/llm"
	"github.com/Audi-dask/Overseer/internal/ocr/model"
	"github.com/Audi-dask/Overseer/internal/ocr/session"
	"github.com/Audi-dask/Overseer/internal/ocr/stdout"
	"github.com/Audi-dask/Overseer/internal/ocr/telemetry"
...
func (r *Runner) RunPerFile(ctx context.Context, messages []llm.Message, newPath string) (bool, error) {
	...
	for toolReqCount > 0 {
		...
		if len(calls) == 0 {
			fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] No tool calls parsed for %s, retrying...\n", newPath)
		...
		if !hasValidResult {
			...
			if consecutiveEmptyRounds >= maxConsecutiveEmptyRounds {
				fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Too many empty retries for %s, stopping.\n", newPath)
				break
			}
			fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] No valid tool results for %s, retrying...\n", newPath)
		} else {
			consecutiveEmptyRounds = 0
		}
		...
		if !succeed {
			fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Context compression exceeded threshold for %s, stopping.\n", newPath)
			break
		}
	}

	if toolReqCount <= 0 {
		fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Max tool requests reached for %s.\n", newPath)
	}
	return false, nil
}
...
func (r *Runner) executeToolCall(ctx context.Context, newPath string, call llm.ToolCall, rec *session.TaskRecord) tool.TaskCheckpoint {
	...
	if !t.IsKnown() {
		...
		telemetry.PrintToolCallStarted(ctx, call.Function.Name, dynArgs)
		...
		if err != nil {
			...
			telemetry.PrintToolCallError(ctx, call.Function.Name, err)
		...
		telemetry.PrintToolCallFinished(ctx, call.Function.Name, dur)
		if rec != nil {
	...
	if t == tool.CodeComment {
		telemetry.PrintToolCallStarted(ctx, t.Name(), args)
		...
		if r.deps.CommentWorkerPool != nil {
			...
			toolName := t.Name()
			pool.SubmitFor(ctx, newPath, func() ([]model.LlmComment, error) {
				defer func() {
					dur := time.Since(startTime)
					telemetry.RecordToolResult(toolSpan, toolName, dur.Milliseconds(), nil)
					toolSpan.End()
					telemetry.PrintToolCallFinished(ctx, toolName, dur)
				}()
		...
		telemetry.PrintToolCallFinished(ctx, t.Name(), dur)
		if rec != nil {
	...
	if err != nil {
		telemetry.PrintToolCallError(ctx, t.Name(), err)
...
func (r *Runner) addNextMessage(ctx context.Context, assistantContent string, toolCalls []llm.ToolCall, results []tool.ToolCallResult, messages *[]llm.Message, filePath string, st *compressionState) bool {
	...
	if CountMessagesTokens(*messages) > warnLimit {
		...
		if *messages, err = r.runCompression(ctx, *messages, filePath); err != nil {
			...
			fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Memory compression failed: %v\n", err)
		}
	}
