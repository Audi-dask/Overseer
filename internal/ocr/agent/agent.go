...
import (
	...
	"github.com/Audi-dask/Overseer/internal/ocr/config/rules"
	...
	"github.com/Audi-dask/Overseer/internal/ocr/llmloop"
...
type Args struct {
	...
	// PathExcluded lets the host service apply its own review firewall before
	// any file content is sent to the LLM.
...
func (a *Agent) Run(ctx context.Context) ([]model.LlmComment, error) {
	...
	fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] %d file(s) changed, reviewing %d in %s\n", totalChanged, reviewCount, filepath.Base(a.args.RepoDir))

	a.diffs = a.filterDiffs(ctx, a.diffs)

	if len(a.diffs) == 0 {
		fmt.Fprintln(stdout.WriterCtx(ctx), "[ocr] No supported files changed. Skipping review.")
	...
	telemetry.Event(ctx, "review.started",
		...
		telemetry.AnyToAttr("repo.dir", filepath.Base(a.args.RepoDir)))
...
func (a *Agent) dispatchSubtasks(ctx context.Context) ([]model.LlmComment, error) {
	...
	a.diffs = a.filterLargeDiffs(ctx, a.diffs)
	if len(a.diffs) == 0 {
		return nil, fmt.Errorf("all diffs filtered out by token size")
	}
	toDispatch := a.applyResume(ctx, a.diffs)
		...
		go func(d model.Diff) {
			...
			defer func() {
				if r := recover(); r != nil {
					...
					fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Subtask panic for %s: %v\n%s\n", d.NewPath, r, debug.Stack())
			...
			if err != nil {
				...
				fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Subtask error for %s: %v\n", d.NewPath, err)
...
func (a *Agent) applyResume(ctx context.Context, diffs []model.Diff) []model.Diff {
	...
	fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Resume %s: reusing %d file(s), reviewing %d file(s)\n", resume.SessionID, reused, rerun)
	return toDispatch
}
...
func (a *Agent) executeSubtask(ctx context.Context, d model.Diff) (bool, string, error) {
	...
	if a.args.Template.PlanTask != nil && len(a.args.Template.PlanTask.Messages) > 0 && threshold > 0 && changeLines < int64(threshold) {
		fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Skipping plan phase for %s (%d lines < threshold %d)\n", newPath, changeLines, threshold)
		...
		if err != nil {
			fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Plan phase failed for %s: %v (continuing without plan)\n", newPath, err)
	...
	if tokenCount > tokenLimit {
		...
		fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] WARNING: %s for %s\n", msg, newPath)
	...
	mainCompleted, err := func() (bool, error) {
		...
		if err != nil {
			mainSpan.SetStatus(telemetry.StatusError, err.Error())
...
func (a *Agent) executeReviewFilter(ctx context.Context, d model.Diff, newPath string) {
	...
	if err != nil {
		...
		fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Review filter failed for %s: %v\n", newPath, err)
	...
	indices := parseFilterResponse(ctx, resp.Content(), len(comments))
	...
	fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Review filter removed %d comment(s) for %s\n", len(indices), newPath)
}
...
func parseFilterResponse(ctx context.Context, raw string, total int) map[int]struct{} {
	...
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		...
		fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Review filter: failed to parse LLM response: %v, raw: %s\n", err, preview)
		return nil
	}
...
func (a *Agent) filterLargeDiffs(ctx context.Context, diffs []model.Diff) []model.Diff {
	...
	for _, d := range diffs {
		...
		if tokens > limit {
			fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Skipping %s (~%d tokens exceeds 80%% of max_tokens(%d))\n",
	...
	if skipped > 0 {
		fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Pre-filtered %d file(s) exceeding 80%% of max_tokens\n", skipped)
	}
	return kept
}
...
func (a *Agent) filterDiffs(ctx context.Context, diffs []model.Diff) []model.Diff {
	var kept []model.Diff
	skipped := 0
	...
	for _, d := range diffs {
		...
		if !a.shouldReview(d) {
			if d.IsBinary {
				fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Skipping %s — binary file\n", path)
			} else {
				fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Skipping %s — filtered by path/extension rules\n", path)
			}
			skipped++
			continue
		}
	...
	if skipped > 0 {
		fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Filtered %d file(s) by include/exclude rules\n", skipped)
	}
	return kept
}
...
func (a *Agent) executePlanPhase(ctx context.Context, newPath, rawDiff, changeFiles, rule string) (string, error) {
	...
	rec.SetResponse(resp, duration)
	a.runner.RecordUsage(resp.Usage)
	fmt.Fprintf(stdout.WriterCtx(ctx), "[ocr] Plan completed for %s\n", newPath)
